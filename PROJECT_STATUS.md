...
> 最后更新：2025（AI 架构研究 + render_canvas Bug 修复 + P2 重构）
> 当前版本：Phase 4.5 — **AI 架构对齐完成** ✅（参考 Claude Code 源码架构进行系统改进）

## ✅ Phase 4.5 — AI 架构对齐（Claude Code 研究成果落地）

> 参考 `ultraworkers/claw-code`（Claude Code Rust 重写版）源码进行系统性架构对齐
> Commits: `2343c3f`（P0/P1）, `70c1696`（P2）

| 优先级 | 改动 | 影响 | commit |
|--------|------|------|--------|
| **P0** | 删除 `openai.rs` 第179行黑名单过滤（`.filter(!= "render_canvas")`） | **根本修复 render_canvas 从未被调用的 Bug** | `2343c3f` |
| **P0** | `tool_choice: required` → `tool_choice: auto` | AI 无需工具时可直接输出文本 | `2343c3f` |
| **P0** | 修复 `execute_tool` 多传第4参数、`canvas_event_was_sent` / `reset_canvas_event_state` 未实现调用 | 清除预存在的编译错误 | `2343c3f` |
| **P1** | `render_canvas` 描述改为动词优先、去掉条件触发语句 | 减少 RLHF 犹豫，AI 更主动调用画图 | `2343c3f` |
| **P2** | 新建 `ai/prompt.rs` → `SocraticPromptBuilder<'a>` | 系统提示词结构化，支持分节 + `DYNAMIC_BOUNDARY` | `70c1696` |
| **P2** | 新建 `ai/client.rs` → `ApiBackend` enum + `from_provider()` 工厂 | 消除 runtime.rs 中重复的 5-branch provider match | `70c1696` |
...
### P3 待办 — AI 运行时增强（源自 Claude Code 架构研究）

> 这些改进基于对 `ultraworkers/claw-code`（Claude Code Rust 重写版）的深度研究，以下是推荐的下一阶段改进。

#### P3.1 会话持久化
- **问题**：当前 `ConversationState.messages` 仅存内存，应用重启后学生上下文全丢
- **方案**：每轮对话结束后序列化 `Vec<Message>` 到 `workspace/session_state.json`，启动时自动加载
- **参考**：claw-code `conversation.rs` `save_to_file()` / `load_from_file()`
- **影响文件**：`ai/runtime.rs`，`commands/ai_commands.rs`

#### P3.2 Token 追踪与成本估算
- **问题**：无法知道每次课堂的 API 费用，无法做预算控制
- **方案**：实现 `UsageTracker`（`tokens_in`, `tokens_out`, `cost_usd` 每轮累加），每阶段结束后写入 `workspace/usage_log.json`
- **参考**：claw-code `token_tracker.rs` `TokenUsage` struct + 每模型单价 map
- **影响文件**：新建 `ai/token_tracker.rs`，`ai/client.rs`（读取 API 响应 usage 字段）

#### P3.3 上下文压缩
- **问题**：超长课堂（>8000 tokens）会超出模型上下文窗口，Prep Phase 历史尤为严重
- **方案**：上下文超阈值时，用轻量 AI 调用对旧消息生成摘要，保留最近 4 条消息原文，注入 `COMPACT_CONTINUATION_PREAMBLE`（告知 AI 上下文已压缩）
- **参考**：claw-code `compact.rs` `compact_session()` + `COMPACT_CONTINUATION_PREAMBLE` 常量
- **影响文件**：新建 `ai/compaction.rs`，`ai/runtime.rs`（在 `run_phase_loop` 开头检测是否需要压缩）

#### P3.4 完整 ApiClient Trait（深层重构）
- **当前状态**：`ApiBackend` enum（P2）已消除 match 重复，但仍需 if/else 分发 claude/openai 的流处理
- **方案**：定义 async trait `ApiClient { send_message(...) -> impl Stream<Item=ContentBlock> }`，`ClaudeClient` 和 `OpenAiClient` 各自实现，`runtime.rs` 中统一调用
- **参考**：claw-code `client.rs` `ApiClient` trait + `ClaudeApiClient` / `OpenAIApiClient` 实现
- **影响文件**：`ai/client.rs`（重写为 trait），`ai/runtime.rs`（改为 trait object dispatch）
