...
#### Apple Vision OCR（macOS 本地免费 OCR）

对于检测为 garbled 的防拷贝 PDF，提供 Apple Vision Framework 本地 OCR：

```
garbled PDF 检测
  → 前端警告框显示 "Vision 识别" 按钮
  → 逐页渲染为 JPEG (pdftoppm 300 DPI)
  → 编译/缓存 Swift CLI 工具 (scripts/apple_ocr.swift)
  → Vision.framework VNRecognizeTextRequest (zh-Hans + en-US)
  → 实时 emit "ocr-progress" 事件（前端进度条）
  → 输出覆盖 Markdown 文件
```

- **优势**：免费、本地隐私、~0.6s/页
- **限制**：仅 macOS、不产出 LaTeX 格式数学公式
- **更高质量方案**：AI Vision OCR (GPT-4o/Claude) 可产出结构化 LaTeX
...
用户点击 "Login with GitHub"
  → 生成 PKCE code_verifier + code_challenge (SHA-256)
  → 启动本地 HTTP 服务器 (localhost:random_port)
  → 打开系统浏览器 → GitHub 授权页
  → 用户登录并授权
  → GitHub 重定向到 localhost:PORT/callback?code=XXX
  → 本地服务器捕获 code，PKCE 交换为 access_token
  → 存入凭证存储，关闭本地服务器
```
