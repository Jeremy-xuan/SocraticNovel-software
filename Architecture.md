...
| PDF 转换 | **pdftotext / poppler + AI Vision** | 教材 PDF 转 Markdown（乱码检测 + AI Vision OCR） |
...
> **macOS Overlay Title Bar**：`titleBarStyle: "Overlay"`（PascalCase，Tauri 2 要求），Traffic light `{ x: 16, y: 12 }`。App.tsx 顶部 32px 透明拖动区域（`-webkit-app-region: drag`），需 `core:window:allow-start-dragging` 权限。全部 13 个页面根容器 `pt-8` padding。
...
#### Meta Prompt 问卷向导（"从零创建"选项）

5 步问卷化引导，替代原 AI 逐步提问（10+ 轮对话压缩为 0 轮）：

```
Step 1: 学科设置 (StepSubject)
  - 选择学科 / 填写大纲
  - PDF 教材上传（提取 + 质量检测 + 乱码警告）
  - 教学模式：标准（无固定剧情线）/ 小说 Beta（用户提供故事参考）

Step 2: 角色设计 (StepCharacters)
  - 20 个动漫角色预设 / 自定义 / 原创
  - 每角色：名字/性别/年龄/外观/教学风格/性格/背景

Step 3: 故事设置 (StepStory)
  - 小说模式子选项："引用现有作品" / "自由描述体验"
  - 故事背景选择：到达方式（4 卡片）+ 教学动机（4 卡片）

Step 4: AI 对话式世界观构建 (StepWorldChat)
  - AI Chat 界面引导 4 个决策（地点/到达/动机/超自然）
  - 快速回复按钮（AI 回复中 [选项:xxx] 标记）
  - AI 输出结构化 JSON → 自动填充 WorldSetting
  - 无 API Key fallback → 传统卡片 UI

Step 5: 确认与创建 (StepReview)
  - 已上传材料汇总
  - 未上传材料确认弹窗
  - Workspace 创建 + PDF 延迟复制
```
...
│   ├── openai.rs               # OpenAI API（含 GPT-5.x max_completion_tokens 兼容）
...
│   └── store.rs                # SQLite
└── commands/
    └── mod.rs                  # Tauri IPC 命令（含 simple_chat 无状态非流式调用）
```
...
```
用户上传 PDF（问卷向导 Step 1 或 Settings 导入页）
  → 调用 pdftotext (poppler) 提取文本（fallback: pdf-extract 纯 Rust）
  → 多采样质量检测（4 个位置: 10%/25%/50%/75%）
    • 常见英文单词检测 (50% 权重)
    • 大写字母占比异常 (30% 权重)
    • 连续辅音字母簇 (20% 权重)
  → quality_score < 0.5 → 标记为 garbled（防拷贝字体编码）
  → 生成 Markdown → 存入 workspace/materials/
  → AI 通过 read_file 访问
```

macOS 可通过 `brew install poppler` 获取 pdftotext。App 首次使用 PDF 功能时检测并提示安装。

#### AI Vision OCR（替代已移除的 Apple Vision）

对于检测为 garbled 的防拷贝 PDF，使用 AI Vision API 进行 OCR：

```
garbled PDF 检测
  → 前端警告横幅显示 "AI Vision 识别" 按钮
  → Pill-style 模型选择器选择 Vision 模型
  → 逐页渲染为图片（PDFium / pdftoppm）
  → 调用 AI Vision API（GPT-4o-mini Vision 等，任意支持 vision 的提供商）
  → AI 返回结构化文本（含 LaTeX 数学公式）
  → 输出覆盖 Markdown 文件
```

- **优势**：跨平台、正确识别 LaTeX 数学公式（ε₀、π 等）
- **对比 Apple Vision（已移除）**：Apple Vision 数学识别差（ε₀→"80"，π→"m"），已移除 ~250 行后端代码 + `scripts/apple_ocr.swift`
- **模型列表**：共享 `PROVIDER_MODELS`（`src/lib/providerModels.ts`）
...
### 5.3 OAuth 流程（Phase 4）

#### GitHub Models OAuth (Device Flow)

从 PKCE + 本地 HTTP 回调迁移到 **OAuth App Device Flow**，原因：GitHub App tokens (`ghu_`) 不支持 Models API，OAuth App tokens (`gho_`) 无需 scope 即可访问。

```
用户点击 "Login with GitHub"
  → 调用 GitHub Device Flow API: POST https://github.com/login/device/code
  → 返回 device_code + user_code + verification_uri
  → 前端显示 user_code，打开浏览器跳转 verification_uri
  → 用户在浏览器输入 user_code 并授权
  → 后端轮询 POST https://github.com/login/oauth/access_token
  → 获取 access_token (gho_xxx) → 存入凭证存储
```

- **Client ID**：`Ov23liVSIwTUn6fuIwlS`（OAuth App，非 GitHub App）
- **API 端点**：`https://models.inference.ai.azure.com/chat/completions`（OpenAI 兼容）
- **认证**：`Authorization: Bearer <github_oauth_token>`
- **Token 存储**：键名 `"github_token"`，`get_api_key` fallback 查找 `{provider}_token`
- **复用**：完全复用 `OpenAiClient`，仅新增 base URL 分支
- 支持模型：`gpt-4o`、`gpt-4o-mini`、`o3-mini`、`claude-sonnet-4`、`Mistral-Large-2`
...
- **Release 模式**：使用 macOS **Keychain** 加密存储（`keyring` crate + `apple-native` feature）
- **Debug 模式**：使用本地 JSON 文件存储（`~/Library/Application Support/SocraticNovel/dev_credentials.json`），避免每次编译后 macOS Keychain 因签名变化弹出密码确认
- 统一抽象层 `credential_store.rs`：通过 `#[cfg(debug_assertions)]` 编译期切换存储后端
- 不存明文到代码仓库，不写数据库
- 卸载 App 不删除密钥（保护用户数据）
...
| Emoji → SVG 图标 | ✅ | 全应用 Heroicons SVG 替代 emoji，~80+ i18n 键 |
| Model Selector 重设计 | ✅ | Workspace 切换器风格下拉 + 品牌 SVG 图标 + `PROVIDER_MODELS` 共享数据源 |
| macOS Overlay Title Bar | ✅ | `titleBarStyle: "Overlay"` + 32px drag region + `pt-8` padding |
| 深色模式修复 | ✅ | Tailwind CSS v4 `@custom-variant dark` class-based variant |
| 教学系统向导 UX 重构 | ✅ | 标准/小说双模式，简化世界观，AI 对话式构建 |
| 对话式世界观构建 | ✅ | `StepWorldChat` AI Chat + `simple_chat` 后端 + 快速回复按钮 |
...
| GPT-5.x 兼容性 | ✅ | `max_tokens` → `max_completion_tokens`，按 model prefix 自动检测 |
...
| 任务 | 状态 | 说明 |
|------|------|------|
| GitHub OAuth | ✅ | OAuth App Device Flow（替代 PKCE），Client ID: `Ov23liVSIwTUn6fuIwlS` |
| Dev 模式凭证存储 | ✅ | `#[cfg(debug_assertions)]` 编译期切换，debug JSON / release Keyring |
| PDF AI Vision OCR | ✅ | 替代 Apple Vision，GPT-4o-mini Vision 等跨平台 OCR |
